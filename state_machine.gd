...
## Animation player adapter for automatic animation playback.
...
## Sets up the animation system with an AnimatedSprite2D or AnimationPlayer.
##
## @param what: AnimatedSprite2D or AnimationPlayer node
...
class State:
	...
	## Configures animation playback for this state.
	##
...
class Transition:
	...
	func _init(from_id: int, to_id: int, cond: Callable, min_time: float) -> void:
		from = from_id
		to = to_id
		condition = cond
		override_min_time = min_time
...
## Configuration for automatic animation playback in states.
## Stores animation settings and handles playback through adapters.
class AnimationConfig:
	...
	## Blend time when transitioning to this animation
	var custom_blend: float = 0
	
	## Whether the animation should loop
	var loop: bool
	
	## Optional callback when animation finishes: Callable()
	var on_finished: Callable
	
	## Plays the configured animation using the given animator.
	##
	## @param animator: Animator instance to use for playback
	func play_animation(animator: Animator) -> void:
		if animator != null:
			animator.play_animation(anim_name, speed, custom_blend, loop, on_finished)
	
	## Internal constructor for AnimationConfig.
	##
	## @param name: Animation name
	## @param anim_speed: Playback speed
...
## Adapter system for different animation player types.
## Provides a unified interface for AnimatedSprite2D and AnimationPlayer.
class Animator:
	## The actual adapter instance (AnimatedSprite2DAdapter or AnimationPlayerAdapter)
	var adapter: Variant = null
	
	## Creates an animator with the appropriate adapter for the player type.
	##
	## @param player: AnimatedSprite2D or AnimationPlayer node
	...
	## Plays an animation with the configured settings.
	##
	## @param anim_name: Name of the animation
	## @param speed: Playback speed multiplier
	## @param custom_blend: Blend time
	## @param loop: Whether to loop
	## @param on_finished: Callback when animation finishes
...
## Adapter for Godot's AnimationPlayer node.
## Handles animation playback, looping, and finish callbacks.
class AnimationPlayerAdapter:
	## Reference to the AnimationPlayer node
	var player: AnimationPlayer
	
	## Maps animation names to their finish callbacks
	var finish_callbacks: Dictionary[String, Callable] = {}
	
	## Creates adapter and connects to animation_finished signal.
	##
	## @param anim_player: AnimationPlayer node to wrap
	...
	## @param speed: Playback speed multiplier
	## @param blend: Blend time for transition
	## @param loop: Whether animation should loop
	## @param on_finished: Callback to execute when animation finishes
	...
	##
	## @param anim_name: Name of the animation that finished
...
## Adapter for Godot's AnimatedSprite2D node.
## Handles sprite animation playback, looping, and finish callbacks.
class AnimatedSprite2DAdapter:
	...
	## Creates adapter and connects to animation_finished signal.
	##
	## @param what: AnimatedSprite2D node to wrap
	...
	## Plays an animation with specified settings.
	## Note: blend parameter is ignored for AnimatedSprite2D.
	##
	## @param anim_name: Name of animation to play
	## @param speed: Playback speed multiplier
	## @param _blend: Unused for AnimatedSprite2D
	## @param loop: Whether animation should loop
	## @param on_finished: Callback to execute when animation finishes
	func play_animation(anim_name: String, speed: float, _blend: float, loop: bool, on_finished: Callable) -> void:
